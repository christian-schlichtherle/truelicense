...
class LicenseConsumerServiceITSuite extends JerseyTest { this: TestContext =>
  ...
  final lazy val reference = new KeyWithLicenseHolder(vendorManager, license)
  final def licenseClass = reference.license.getClass
  ...
  def assertView(mediaType: MediaType) {
    viewAs(mediaType) should equal (reference.license)
  }
  ...
  def assertVerify(mediaType: MediaType) {
    verifyAs(mediaType) should equal (reference.license)
  }
